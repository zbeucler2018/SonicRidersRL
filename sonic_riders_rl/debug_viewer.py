...
async function tick(){try{await frame();t.textContent=JSON.stringify(await (await fetch('/telemetry')).json(),null,2)}catch(_){ }setTimeout(tick,80)}send();tick();</script>'''
...
class ViewerState:
    ...
    def controller(self) -> ControllerState:
        ...
        return ControllerState(
            buttons=(GameCubeButton.A.mask if "z" in keys else 0)
            | (GameCubeButton.B.mask if "x" in keys else 0)
            | (GameCubeButton.START.mask if "Enter" in keys else 0),
            left_x=20_000 * (("d" in keys) - ("a" in keys)),
            left_y=20_000 * (("s" in keys) - ("w" in keys)),
            left_trigger=20_000 if "q" in keys else 0,
            right_trigger=20_000 if "e" in keys else 0,
        )
...
def _handler(state: ViewerState):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            if self.path == "/keys":
                keys = set(json.loads(self.rfile.read(int(self.headers["Content-Length"]))))
                with state.lock: state.keys = keys
